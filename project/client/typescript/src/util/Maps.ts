...
class KeySet<K, V> implements ReadonlySet<K> {
	public constructor(private map: ReadonlyMap<K, V>) { }

	forEach(callbackfn: (value: K, value2: K, set: ReadonlySet<K>) => void, thisArg?: unknown): void {
		this.map.forEach((_, value) => callbackfn.call(thisArg, value, value, this));
	}
...
Map.prototype.keySet = function () {
	let cached = keySets.get(this);
	if (cached) return cached;
	cached = new KeySet(this);
	keySets.set(this, cached);
	return cached;
};